...
void AlbumManager::createAlbum()
{
	...
	if ( m_dataAccess.doesAlbumExists(name) ) {
...
void AlbumManager::deleteAlbum()
{
	...
	if ( !m_dataAccess.doesAlbumExists(albumName) ) {
...
void AlbumManager::listAlbums()
{
	...
	const std::list<Album>& albums = m_dataAccess.getAlbums();
	std::cout << "Albums list:" << std::endl;
	std::cout << "----------------" << std::endl;

	for (const auto& album : albums) {
		const User& user = m_dataAccess.getUser(album.getOwnerId());
		std::cout << "   + [" << album.getName() << "] - created by " << user.getName() << " (@" << user.getId() <<") on date: "<< album.getCreationDate() << std::endl;
	}
}
